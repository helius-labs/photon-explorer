...
export function NetworkStatusDropdown() {
  ...
  const { data: networkStatus, isLoading: isNetworkLoading } =
    useGetRecentPerformanceSamples(isHovered);
  const { data: priorityFeeLevels, isLoading: isFeeLoading } =
    useGetPriorityFeeEstimate(accountKeys, isHovered);
      ...
      <HoverCardTrigger asChild>
        ...
        >
          <div className="min-w-24 px-1 flex items-center justify-between">
            {clusterName(cluster)}
            <div
              className={`w-2 h-2 mr-2 rounded-full ${networkConditionColor} animate-pulse`}
            ></div>
          </div>
        </Button>
      </HoverCardTrigger>
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex items-center space-x-2 transition duration-300 ease-in-out transform-gpu hover:scale-105">
                      <div
                        className={`w-3 h-3 rounded-full ${networkConditionColor}`}
                      ></div>
          ...
          <div>
            ...
            <div className="grid gap-4">
              ...
              <div
                ...
                className={`cursor-pointer rounded-md px-4 py-2 border text-center ${cluster === Cluster.Custom ? "ring-1" : ""}`}
              >
                Custom
              </div>
                ...
                <>
                  ...
                  <div className="grid w-full max-w-sm items-center gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCustomCompressionEndpoint(e.target.value)
                      }
                    />
                  </div>
                </>
