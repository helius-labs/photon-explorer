...
import { useGetAverageTps } from '@/hooks/web3';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';
import { CogIcon } from 'lucide-react';

export function NetworkStatusDropdown() {
  const { data: avgTps, isLoading: isLoadingTps } = useGetAverageTps();

  const averageTps = avgTps !== undefined ? Math.round(avgTps).toLocaleString('en-US') : 'N/A';
          ...
          <div className="flex justify-between mb-4">
            <div>
              ...
              <div className="text-lg font-semibold">
                {isLoadingTps ? 'Loading...' : averageTps}
              </div>
            </div>
            <div>
